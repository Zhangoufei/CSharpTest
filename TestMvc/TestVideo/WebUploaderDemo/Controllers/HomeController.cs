...
namespace WebUploaderDemo.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            string fileDataJson = Request.Cookies["fileDataJson"].Value;//文件信息json
            FileDataInfo fileData = Tools.JsonTransformation.JsonToEntity<FileDataInfo>(fileDataJson);//文件信息实体
            fileInfoList.Add(fileData);
            
            HttpPostedFileBase file = Request.Files["file"];
            string tempPath = Server.MapPath("~/Content/TempFile/");//临时保存路径
            string filePath = Server.MapPath("~/Content/Files/");//保存路径
            string ZSFilePath = filePath + DateTime.Now.ToString("yyyyMMddHHmmss") + Path.GetExtension(fileData.fileName);//最终保存路劲 
