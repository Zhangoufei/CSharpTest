...
namespace BonSite.Core
{
    ...
    public class WebHelper
    {
        ...
        public static string GetCookie(string name, string key)
        {
            ...
            {
                string v = HttpUtility.UrlDecode(cookie.Values["" + key + ""].ToString());
                //string v = cookie[key];
                if (v != null)
                    return v;
            }
        ...
        public static string GetCookie2(string name, string key)
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies[name];
        ...
        /// <param name="postData">发送数据</param>
        /// <returns></returns>
        public static string SendRequestData(string url, string method, StringBuilder param)
        {
            if (!(url.Contains("http://") || url.Contains("https://")))
            {
                url = "http://" + url;
        ...
        //获得请求数据
        public static string PostWebReq(string PostUrl)
        {
            string ret = string.Empty;
            if (!(PostUrl.Contains("http://") || PostUrl.Contains("https://")))
            {
