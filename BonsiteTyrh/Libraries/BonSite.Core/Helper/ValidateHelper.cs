...
namespace BonSite.Core
{
    ...
    {
        //邮件正则表达式
        private static Regex _emailregex = new Regex(@"([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])", RegexOptions.IgnoreCase);
        //手机号正则表达式
        ...
        //密码正则表达式
        private static Regex _passwordregex = new Regex(@"^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,21}$");
        /// <summary>
        /// 是否为邮箱名
        /// </summary>
        ...
        public static bool IsVideoFileName(string fileName)
        {
            ...
            string tempFileName = fileName.Trim().ToLower();
            string extension = tempFileName.Substring(tempFileName.LastIndexOf("."));
            return extension == ".mp4" || extension == ".png" || extension == ".bmp" || extension == ".jpg" || extension == ".jpeg" || extension == ".gif";
        }
        //获取视频第一帧
        public static string CatchImg(string vFileName)
        {
            ...
            {
                //视频存放路径
                string videopath = IOHelper.GetMapPath("/upload/article/video/source/");
                string epath = videopath + vFileName;
                //图片存放路径
                string imgpath = IOHelper.GetMapPath("/upload/article/show/thumb640_480/");
                string imgepath = imgpath + vFileName;
                //服务器地址
                string serverPath = HttpContext.Current.Server.MapPath(System.Web.HttpContext.Current.Request.ApplicationPath.ToString());
                vFileName = videopath.Replace(serverPath, "") + vFileName;
                string ffmpeg = "/Content/";
                //ffmpeg = HttpContext.Current.Server.MapPath(ffmpeg);
                ffmpeg = IOHelper.GetMapPath(ffmpeg) + "ffmpeg.exe";
                //ffmpeg = ffmpeg.Replace(serverPath, "");
                bool bbb = File.Exists(ffmpeg);
                bool ccc = File.Exists(epath);
                if ((!File.Exists(ffmpeg)) || (!File.Exists(epath)))
                {
                    return "";
                }
                string flv_img = Path.ChangeExtension(imgepath, ".jpg");
                string FlvImgSize = "340x200";
                System.Diagnostics.ProcessStartInfo startInfo = new System.Diagnostics.ProcessStartInfo(ffmpeg);
                startInfo.WindowStyle = System.Diagnostics.ProcessWindowStyle.Hidden;
                startInfo.Arguments = " -i " + epath + " -y -ss 00:00:10 -f image2 -t 0.001  " + flv_img;
                try
                {
                ...
                {
                    string []imgdd=flv_img.Split('\\');
                    return imgdd[imgdd.Length - 1]; 
                }
                return "";
            }
            catch
