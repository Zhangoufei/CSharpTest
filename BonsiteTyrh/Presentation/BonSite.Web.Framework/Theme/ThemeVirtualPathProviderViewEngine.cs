...
namespace BonSite.Web.Framework
{
    ...
    public abstract class ThemeVirtualPathProviderViewEngine : VirtualPathProviderViewEngine
    {
        ...
        {
            //判断主题是否支持移动访问且访问是来自移动
            bool mobile = (BSConfig.SysConfig.IsMobile == 1) && WebHelper.IsMobile();

            //如果主题支持移动访问且访问是来自移动，则构建一个新的视图名称
            ...
            //如果主题支持移动访问且访问是来自移动且没有对应视图文件时采用原视图名称解析
        ...
        public override ViewEngineResult FindPartialView(ControllerContext controllerContext, string partialViewName, bool useCache)
        {
            ...
            //如果主题支持移动访问且访问是来自移动，则构建一个新的分部视图名称
            ...
            //如果主题支持移动访问且访问是来自移动且没有对应分部视图文件时采用原分部视图名称解析
            if (mobile && (result == null || result.View == null))
        ...
        private string GetPath(ControllerContext controllerContext, string name, string controllerName, string cacheKeyPrefix, bool useCache, bool mobile, out string[] searchedLocations)
        {
            ...
            //主题
            string theme = BSConfig.SysConfig.ThemeName;
            //是否为特殊路径的标识
