...
namespace BonSite.Web.Framework
{
    public class BaseWebController : Controller
    {
        ...
        protected override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            //商城已经关闭
            if (WorkContext.SiteConfig.IsClosed == 1 && WorkContext.AdminGid == 1 && WorkContext.PageKey != "/account/login" && WorkContext.PageKey != "/account/logout")
            {
                filterContext.Result = PromptView(WorkContext.SiteConfig.CloseReason);
                return;
            }

            //当前时间为禁止访问时间
            if (ValidateHelper.BetweenPeriod(WorkContext.SiteConfig.BanAccessTime) && WorkContext.AdminGid == 1 && WorkContext.PageKey != "/account/login" && WorkContext.PageKey != "/account/logout")
            {
                filterContext.Result = PromptView("当前时间不能访问本网站");
                return;
            }

            //当用户ip在被禁止的ip列表时
            if (ValidateHelper.InIPList(WorkContext.IP, WorkContext.SiteConfig.BanAccessIP))
            {
                filterContext.Result = PromptView("您的IP被禁止访问本网站");
                return;
            }

            //当用户ip不在允许的ip列表时
            if (!string.IsNullOrEmpty(WorkContext.SiteConfig.AllowAccessIP) && !ValidateHelper.InIPList(WorkContext.IP, WorkContext.SiteConfig.AllowAccessIP))
            {
                filterContext.Result = PromptView("您的IP被禁止访问本网站");
                return;
            }
