        ...
        fireEvent: function() {
            ...
            for (var i = 0, ti; ti = types[i++]; ) {
                ...
                if (listeners) {
                    ...
                    while (k--) {
                        if (!listeners[k])
                            continue;t = listeners[k].apply(this, arguments);
                        if (t === true) {
                            return t
                        }
                        if (t !== undefined) {
    ...
    var domUtils = dom.domUtils = {
        ...
        breakParent: function(node, parent) {
            ...
            do {
                ...
                clone = parentClone
            } while (parent !== parentClone);tmpNode = parent.parentNode;
        ...
        function json2str(json) {
            ...
            for (var i in json) {
                if (i == "method" || i == "timeout" || i == "async")
                    continue;if (!((typeof json[i]).toLowerCase() == "function" || (typeof json[i]).toLowerCase() == "object")) {
            ...
            if (img && /img/i.test(img.tagName) && img.className != "edui-faked-video" && !img.getAttribute("word_img")) {
                ...
                if (opt.length == 1) {
                    ...
                    for (var i = 0; ci = opt[i++]; ) {
                        str = "<p " + (ci["floatStyle"] == "center" ? 'style="text-align: center" ' : "") + '><img src="' + ci.src + '" ' + (ci.width ? 'width="' + ci.width + '" ' : "") + (ci._src ? ' _src="' + ci._src + '" ' : "") + (ci.height ? ' height="' + ci.height + '" ' : "") + ' style="' + (ci["floatStyle"] && ci["floatStyle"] != "center" ? "float:" + ci["floatStyle"] + ";" : "") + (ci.border || "") + '" ' + (ci.title ? ' title="' + ci.title + '"' : "") + " /></p><p style='text-align:center'>"+ci.title+"</p>";
                        html.push(str)
                    }
                }
        ...
        UETable.prototype = {
            ...
            splitToRows: function(cell) {
                ...
                for (var i = rowIndex, endRow = rowIndex + cellInfo.rowSpan; i < endRow; i++) {
                    if (i == rowIndex)
                        continue;var tableRow = this.table.rows[i]
            ...
            splitToCols: function(cell) {
                ...
                for (var j = colIndex, endCol = colIndex + cellInfo.colSpan; j < endCol; j++) {
                    if (j == colIndex)
                        continue;var tableRow = this.table.rows[rowIndex]
